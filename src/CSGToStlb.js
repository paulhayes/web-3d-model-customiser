...
function serialize (CSG, options) {
  ...
  var numtriangles = 0
  CSG.polygons.forEach(function (p,i) {
    var numvertices = p.vertices.length
    var thisnumtriangles = (numvertices >= 3) ? numvertices - 2 : 0
    numtriangles += thisnumtriangles
    options && options.statusCallback && options.statusCallback({progress: 10 * i / CSG.polygons.length})
  })
  ...
  CSG.polygons.map(function (p,i) {
    ...
    options && options.statusCallback && options.statusCallback({progress: 10 + 90 * i / CSG.polygons.length})
  })
