...
function LightGLEngine (containerelement, options) {
  ...
  this.overrideDefaults(this.options,options);
...
LightGLEngine.prototype = {
  ...
  defaults: function () {
    return {
      camera: {
        ...
        angle: {x: -60, y: 60, z: -45},  // view angle about XYZ axis
  ...
  overrideDefaults:function(initial,overrides){
    for(var n in overrides){
      if(n in initial){
        if( typeof(initial[n])==='object' ){
          if(typeof(overrides[n])!=='object'){
            throw new Error(`unable to handle override ${n}, check type being overriden`);
          }
          this.overrideDefaults(initial[n],overrides[n]);
        }
        else {
          initial[n] = overrides[n];
        }
      }
    }
  },
